...
function func() {
  var a = 1,
    ...
    j;
  // 함수 본문...
} //코드 가독성과 중복 변수 정의를 피하기 위해 단일 var 선언을 추천
...
function func () {
  ...
  var myname = 'local';
  console.log(myname);
} //함수 내부 변수가 자동 선언되면서 함수내에서 끌어 올려짐.
// 설령 밑에서 정의 되어 있더라 하더라도..
func();
//위의 코드와 동일한 의미의 코드.
myname = 'global';
function func () {
...
2. 이 프로퍼티 또는 메서드가 이미 존재하는지, 즉 이미 코드 어딘가에 구현되어 있거나,
    지원 브라우저 중 일부 자바스크립트 엔진에 내장되어 있는지 확인한다.
...
// 어쩔 수 없이 사용하게 되면 new Function 방식으로 사용하는게 좋다. 맞나?
